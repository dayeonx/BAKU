...
export default function DepartmentTaskSummary({ onSelectTask }: { onSelectTask: (taskId: string) => void }) {
  ...
  const load = useCallback(async () => {
    ...
    const { data: taskRows } = await supabase
      .from("work_tasks")
      .select("id, department, title, status, due_date")
  ...
  return (
    <div className="grid grid-cols-5 gap-2 sm:gap-3">
      {BOARD_DEPARTMENTS.map((dept) => {
        const deptTasks = tasks.filter((t) => t.department === dept.value && t.status !== "done").sort(compareDue);
        return (
